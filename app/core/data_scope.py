...
多角色取最宽松：all > department > self。超级管理员自动跳过。
...
# 数据范围宽松度排序（用于多角色取最宽松）
_SCOPE_PRIORITY = {"all": 0, "department": 1, "self": 2, "custom": 3}
...
def build_scope_filter(
    *,
    ...
    Args:
        scope: 数据范围值（all / department / self / custom）。
        user_id: 当前用户 ID。
        department_id: 当前用户所在部门 ID。
        user_id_field: 模型中用户 ID 字段名。
        dept_id_field: 模型中部门 ID 字段名。
    """
